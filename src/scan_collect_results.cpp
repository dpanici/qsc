                           ...
                           big j_scan)
{
  ...
  else
  {
    ...
    for (j = 0; j < N_FILTERS; j++)
      filters_combined[j] = filters_local[j];
    for (j = 0; j < N_TIMES; j++)
      timing_combined[j] = timing_local[j];
    ...
    for (j = REJECTED_DUE_TO_R0_CRUDE; j < N_SIGMA_EQ_SOLVES; j++)
      total_rejected += filters[j];
    for (j = 0; j < N_FILTERS; j++)
