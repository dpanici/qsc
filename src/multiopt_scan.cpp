...
void MultiOptScan::init()
{
  ...
  {
    if (params_n[j] < 2)
      throw std::runtime_error("params_n values must all be >= 2");
  ...
  if (verbose > -1 && proc0)
    std::cout << "Total number of points in scan: " << n_scan_all << std::endl;

  axis_nmax_plus_1 = mo_ref.opts[0].q.R0c.size();
  for (j = 0; j < mo_ref.opts.size(); j++)
    axis_nmax_plus_1 += mo_ref.opts[j].fourier_refine;
...
void MultiOptScan::scan()
{
  ...
  if (proc0)
  {
    ...
    if (verbose > 0)
      std::cout << "proc0 about to allocate big arrays." << std::endl;
    ...
    for (j = n_procs - 1; j < n_scan_all; j++)
    {
      ...
      if (verbose > 0)
        std::cout << "proc0: I see that proc " << proc_that_finished << " just finished, so I'll send it index " << j << std::endl;
      intbuf[0] = j;
  ...
  else
  {
    ...
    while (keep_going)
    {
      ...
      else
      {
        ...
        if (verbose > 0)
          std::cout << "proc " << mpi_rank << " is returning results for index " << intbuf[0] << std::endl;
...
int MultiOptScan::proc0_recv()
{
  ...
  if (filters_local[KEPT] > 0)
    n_solves_kept[proc_that_finished]++;
...
void MultiOptScan::eval_scan_index(int j_scan)
{
    ...
    for (stage = stage_min; stage < stage_max; stage++)
    {
      ...
      for (int mode_num = 1; mode_num < axis_nmax_plus_1; mode_num++)
      {
        ...
        if (params[j].compare("R0c" + mode_num_str) == 0)
        {
          ...
          if (verbose > 1)
            std::cout << "Setting R0c[" << mode_num << "] for opt stage " << stage << " to " << val << std::endl;
          if (stage != 0)
            throw std::runtime_error("In a multiopt_scan, R0c should only be set at stage 0.");
          found_param = true;
        }
        else if (params[j].compare("Z0s" + mode_num_str) == 0)
        {
          ...
          if (verbose > 1)
            std::cout << "Setting Z0s[" << mode_num << "] for opt stage " << stage << " to " << val << std::endl;
          if (stage != 0)
            throw std::runtime_error("In a multiopt_scan, Z0s should only be set at stage 0.");
          found_param = true;
        }
      }
      ...
      else if (params[j].compare("weight_iota") == 0)
      {
        ...
        if (verbose > 1)
          std::cout << "Setting weight_iota for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("target_iota") == 0)
      {
        mo.opts[stage].target_iota = val;
        if (verbose > 1)
          std::cout << "Setting target_iota for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_elongation") == 0)
      {
        mo.opts[stage].weight_elongation = val;
        if (verbose > 1)
          std::cout << "Setting weight_elongation for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_B20") == 0)
      {
        mo.opts[stage].weight_B20 = val;
        if (verbose > 1)
          std::cout << "Setting weight_B20 for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_r_singularity") == 0)
      {
        mo.opts[stage].weight_r_singularity = val;
        if (verbose > 1)
          std::cout << "Setting weight_r_singularity for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_grad_B") == 0)
      {
        mo.opts[stage].weight_grad_B = val;
        if (verbose > 1)
          std::cout << "Setting weight_grad_B for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_grad_grad_B") == 0)
      {
        mo.opts[stage].weight_grad_grad_B = val;
        if (verbose > 1)
          std::cout << "Setting weight_grad_grad_B for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_axis_length") == 0)
      {
        mo.opts[stage].weight_axis_length = val;
        if (verbose > 1)
          std::cout << "Setting weight_axis_length for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("target_axis_length") == 0)
      {
        mo.opts[stage].target_axis_length = val;
        if (verbose > 1)
          std::cout << "Setting target_axis_length for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_standard_deviation_of_R") == 0)
      {
        mo.opts[stage].weight_standard_deviation_of_R = val;
        if (verbose > 1)
          std::cout << "Setting weight_standard_deviation_of_R for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_XY2") == 0)
      {
        mo.opts[stage].weight_XY2 = val;
        if (verbose > 1)
          std::cout << "Setting weight_XY2 for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_Z2") == 0)
      {
        mo.opts[stage].weight_Z2 = val;
        if (verbose > 1)
          std::cout << "Setting weight_Z2 for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_XY3") == 0)
      {
        mo.opts[stage].weight_XY3 = val;
        if (verbose > 1)
          std::cout << "Setting weight_XY3 for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_XY2Prime") == 0)
      {
        mo.opts[stage].weight_XY2Prime = val;
        if (verbose > 1)
          std::cout << "Setting weight_XY2Prime for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_XY2PrimePrime") == 0)
      {
        mo.opts[stage].weight_XY2PrimePrime = val;
        if (verbose > 1)
          std::cout << "Setting weight_XY2PrimePrime for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_Z2Prime") == 0)
      {
        mo.opts[stage].weight_Z2Prime = val;
        if (verbose > 1)
          std::cout << "Setting weight_Z2Prime for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_XY3Prime") == 0)
      {
        mo.opts[stage].weight_XY3Prime = val;
        if (verbose > 1)
          std::cout << "Setting weight_XY3Prime for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_XY3PrimePrime") == 0)
      {
        mo.opts[stage].weight_XY3PrimePrime = val;
        if (verbose > 1)
          std::cout << "Setting weight_XY3PrimePrime for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_B20_mean") == 0)
      {
        mo.opts[stage].weight_B20_mean = val;
        if (verbose > 1)
          std::cout << "Setting weight_B20_mean for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_d2_volume_d_psi2") == 0)
      {
        mo.opts[stage].weight_d2_volume_d_psi2 = val;
        if (verbose > 1)
          std::cout << "Setting weight_d2_volume_d_psi2 for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("max_d2_volume_d_psi2") == 0)
      {
        mo.opts[stage].max_d2_volume_d_psi2 = val;
        if (verbose > 1)
          std::cout << "Setting max_d2_volume_d_psi2 for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("weight_DMerc_times_r2") == 0)
      {
        mo.opts[stage].weight_DMerc_times_r2 = val;
        if (verbose > 1)
          std::cout << "Setting weight_DMerc_times_r2 for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("min_DMerc_times_r2") == 0)
      {
        mo.opts[stage].min_DMerc_times_r2 = val;
        if (verbose > 1)
          std::cout << "Setting min_DMerc_times_r2 for opt stage " << stage << " to " << val << std::endl;
      }
      else if (params[j].compare("eta_bar") == 0)
      {
        mo.opts[stage].q.eta_bar = val;
        if (verbose > 1)
          std::cout << "Setting eta_bar for opt stage " << stage << " to " << val << std::endl;
        if (stage != 0)
          throw std::runtime_error("In a multiopt_scan, eta_bar should only be set at stage 0.");
      }
      else if (params[j].compare("p2") == 0)
      {
        mo.opts[stage].q.p2 = val;
        if (verbose > 1)
          std::cout << "Setting p2 for opt stage " << stage << " to " << val << std::endl;
        if (stage != 0)
          throw std::runtime_error("In a multiopt_scan, p2 should only be set at stage 0.");
      }
      else if (params[j].compare("B2c") == 0)
      {
        mo.opts[stage].q.B2c = val;
        if (verbose > 1)
          std::cout << "Setting B2c for opt stage " << stage << " to " << val << std::endl;
        if (stage != 0)
          throw std::runtime_error("In a multiopt_scan, B2c should only be set at stage 0.");
      }
      else if (params[j].compare("B2s") == 0)
      {
        mo.opts[stage].q.B2s = val;
        if (verbose > 1)
          std::cout << "Setting B2s for opt stage " << stage << " to " << val << std::endl;
        if (stage != 0)
          throw std::runtime_error("In a multiopt_scan, B2s should only be set at stage 0.");
      }
      else
      {
  ...
  if (!keep_all)
  {
    ...
    if (mo.opts[index].q.grid_min_R0 < min_R0_to_keep)
    {
      ...
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to min_R0." << std::endl;
    ...
    if (std::abs(mo.opts[index].q.iota) < min_iota_to_keep)
    {
      ...
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to iota." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed iota filter." << std::endl;

    if (mo.opts[index].q.grid_max_elongation > max_elongation_to_keep)
    {
      filters_local[REJECTED_DUE_TO_ELONGATION]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to elongation. "
                  << mo.opts[index].q.grid_max_elongation << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed elongation filter. "
                << mo.opts[index].q.grid_max_elongation << std::endl;
    ...
    if (mo.opts[index].q.grid_min_L_grad_B < min_L_grad_B_to_keep)
    {
      ...
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to L_grad_B." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed L_grad_B filter." << std::endl;

    if (mo.opts[index].q.B20_grid_variation > max_B20_variation_to_keep)
    {
      filters_local[REJECTED_DUE_TO_B20_VARIATION]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to B20 variation. "
                  << mo.opts[index].q.B20_grid_variation << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed B20 variation filter. "
                << mo.opts[index].q.B20_grid_variation << std::endl;
    ...
    if (mo.opts[index].q.d2_volume_d_psi2 > max_d2_volume_d_psi2_to_keep)
    {
      ...
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to d2_volume_d_psi2." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed d2_volume_d_psi2 filter." << std::endl;

    if (mo.opts[index].q.DMerc_times_r2 < min_DMerc_times_r2_to_keep)
    {
      filters_local[REJECTED_DUE_TO_DMERC]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to DMerc." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed DMerc filter." << std::endl;

    if (mo.opts[index].q.grid_min_L_grad_grad_B < min_L_grad_grad_B_to_keep)
    {
      filters_local[REJECTED_DUE_TO_L_GRAD_GRAD_B]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to L_grad_grad_B." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed L_grad_grad_B filter." << std::endl;

    if (mo.opts[index].q.r_singularity_robust < min_r_singularity_to_keep)
    {
      filters_local[REJECTED_DUE_TO_R_SINGULARITY]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to r_singularity. "
                  << mo.opts[index].q.r_singularity_robust << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed r_singularity filter. "
                << mo.opts[index].q.r_singularity_robust << std::endl;
    ...
    if (mo.opts[index].q.grid_max_XY2 > max_XY2_to_keep)
    {
      ...
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to max XY2." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed max XY2 filter." << std::endl;

    if (mo.opts[index].q.grid_max_Z2 > max_Z2_to_keep)
    {
      filters_local[REJECTED_DUE_TO_MAX_Z2]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to max Z2." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed max Z2 filter." << std::endl;

    if (mo.opts[index].q.grid_max_XY3 > max_XY3_to_keep)
    {
      filters_local[REJECTED_DUE_TO_MAX_XY3]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to max XY3." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed max XY3 filter." << std::endl;

    if (mo.opts[index].q.grid_max_d_XY2_d_varphi > max_d_XY2_d_varphi_to_keep)
    {
      filters_local[REJECTED_DUE_TO_MAX_D_XY2_D_VARPHI]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to max d XY2 / d varphi." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed max d XY2 / d varphi filter." << std::endl;

    if (mo.opts[index].q.grid_max_d_Z2_d_varphi > max_d_Z2_d_varphi_to_keep)
    {
      filters_local[REJECTED_DUE_TO_MAX_D_Z2_D_VARPHI]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to max d Z2 / d varphi." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed max d Z2 / d varphi filter." << std::endl;

    if (mo.opts[index].q.grid_max_d_XY3_d_varphi > max_d_XY3_d_varphi_to_keep)
    {
      filters_local[REJECTED_DUE_TO_MAX_D_XY3_D_VARPHI]++;
      if (verbose > 1)
        std::cout << "Rejecting this configuration due to max d XY3 / d varphi." << std::endl;
      passed_filters = false;
    }
    else if (verbose > 1)
      std::cout << "Passed max d XY3 / d varphi filter." << std::endl;
  }
  if (passed_filters)
    filters_local[KEPT]++;
  ...
  parameters_single[17] = mo.opts[index].q.grid_min_R0;
  parameters_single[18] = mo.opts[index].q.standard_deviation_of_R;
  parameters_single[19] = mo.opts[index].q.standard_deviation_of_Z;
  parameters_single[20] = mo.opts[index].q.grid_max_XY2;
  parameters_single[21] = mo.opts[index].q.grid_max_Z2;
  parameters_single[22] = mo.opts[index].q.grid_max_XY3;
  parameters_single[23] = mo.opts[index].q.grid_max_d_XY2_d_varphi;
  parameters_single[24] = mo.opts[index].q.grid_max_d_Z2_d_varphi;
  parameters_single[25] = mo.opts[index].q.grid_max_d_XY3_d_varphi;
  parameters_single[26] = mo.opts[index].q.grid_max_d2_XY2_d_varphi2;
  parameters_single[27] = mo.opts[index].q.grid_max_d2_XY3_d_varphi2;
  parameters_single[28] = mo.opts[index].q.axis_length;
  parameters_single[29] = mo.opts[index].q.p2;
  parameters_single[30] = mo.opts[0].iter_eta_bar[0];
  parameters_single[31] = mo.opts[0].iter_B2c[0];
  parameters_single[32] = mo.opts[0].iter_B2s[0];

  parameters_single[33] = mo.opts[index].weight_B20;
  parameters_single[34] = mo.opts[index].weight_iota;
  parameters_single[35] = mo.opts[index].target_iota;
  parameters_single[36] = mo.opts[index].weight_elongation;
  parameters_single[37] = mo.opts[index].weight_curvature;
  parameters_single[38] = mo.opts[index].weight_R0;
  parameters_single[39] = mo.opts[index].min_R0;
  parameters_single[40] = mo.opts[index].weight_d2_volume_d_psi2;
  parameters_single[41] = mo.opts[index].max_d2_volume_d_psi2;
  parameters_single[42] = mo.opts[index].weight_DMerc_times_r2;
  parameters_single[43] = mo.opts[index].min_DMerc_times_r2;
  parameters_single[44] = mo.opts[index].weight_XY2;
  parameters_single[45] = mo.opts[index].weight_XY2Prime;
  parameters_single[46] = mo.opts[index].weight_XY2PrimePrime;
  parameters_single[47] = mo.opts[index].weight_Z2;
  parameters_single[48] = mo.opts[index].weight_Z2Prime;
  parameters_single[49] = mo.opts[index].weight_XY3;
  parameters_single[50] = mo.opts[index].weight_XY3Prime;
  parameters_single[51] = mo.opts[index].weight_XY3PrimePrime;
  parameters_single[52] = mo.opts[index].weight_grad_B;
  parameters_single[53] = mo.opts[index].weight_grad_grad_B;
  parameters_single[54] = mo.opts[index].weight_r_singularity;
  parameters_single[55] = mo.opts[index].weight_axis_length;
  parameters_single[56] = mo.opts[index].target_axis_length;
  parameters_single[57] = mo.opts[index].weight_standard_deviation_of_R;
  parameters_single[58] = mo.opts[index].weight_B20_mean;
...
void MultiOptScan::print_status()
{
  ...
  elapsed = end_time - start_time;
  std::cout << std::endl
            << "Status at " << elapsed.count() << " seconds:" << std::endl;
  ...
  for (j = 0; j < n_procs; j++)
    std::cout << " " << attempts_per_proc[j];
  std::cout << std::endl;
  std::cout << "# solves kept on each proc:";
  for (j = 0; j < n_procs; j++)
    std::cout << " " << n_solves_kept[j];
  std::cout << std::endl;

  for (j = 0; j < N_FILTERS; j++)
    filter_fractions[j] = ((qscfloat)filters[j]) / filters[0];
...
void MultiOptScan::filter_global_arrays()
{
  ...
  {
    if (int_parameters[1 + j_global * n_int_parameters_base] < 1)
      continue;
    j++;
    ...
    scan_max_Z2[j] = parameters(21, j_global);
    scan_max_XY3[j] = parameters(22, j_global);
    scan_max_d_XY2_d_varphi[j] = parameters(23, j_global);
    scan_max_d_Z2_d_varphi[j] = parameters(24, j_global);
    scan_max_d_XY3_d_varphi[j] = parameters(25, j_global);
    scan_max_d2_XY2_d_varphi2[j] = parameters(26, j_global);
    scan_max_d2_XY3_d_varphi2[j] = parameters(27, j_global);
    scan_axis_length[j] = parameters(28, j_global);
    scan_p2[j] = parameters(29, j_global);
    scan_initial_eta_bar[j] = parameters(30, j_global);
    scan_initial_B2c[j] = parameters(31, j_global);
    scan_initial_B2s[j] = parameters(32, j_global);

    scan_weight_B20[j] = parameters(33, j_global);
    scan_weight_iota[j] = parameters(34, j_global);
    scan_target_iota[j] = parameters(35, j_global);
    scan_weight_elongation[j] = parameters(36, j_global);
    scan_weight_curvature[j] = parameters(37, j_global);
    scan_weight_R0[j] = parameters(38, j_global);
    scan_target_min_R0[j] = parameters(39, j_global);
    scan_weight_d2_volume_d_psi2[j] = parameters(40, j_global);
    scan_max_d2_volume_d_psi2[j] = parameters(41, j_global);
    scan_weight_DMerc_times_r2[j] = parameters(42, j_global);
    scan_min_DMerc_times_r2[j] = parameters(43, j_global);
    scan_weight_XY2[j] = parameters(44, j_global);
    scan_weight_XY2Prime[j] = parameters(45, j_global);
    scan_weight_XY2PrimePrime[j] = parameters(46, j_global);
    scan_weight_Z2[j] = parameters(47, j_global);
    scan_weight_Z2Prime[j] = parameters(48, j_global);
    scan_weight_XY3[j] = parameters(49, j_global);
    scan_weight_XY3Prime[j] = parameters(50, j_global);
    scan_weight_XY3PrimePrime[j] = parameters(51, j_global);
    scan_weight_grad_B[j] = parameters(52, j_global);
    scan_weight_grad_grad_B[j] = parameters(53, j_global);
    scan_weight_r_singularity[j] = parameters(54, j_global);
    scan_weight_axis_length[j] = parameters(55, j_global);
    scan_target_axis_length[j] = parameters(56, j_global);
    scan_weight_standard_deviation_of_R[j] = parameters(57, j_global);
    scan_weight_B20_mean[j] = parameters(58, j_global);
