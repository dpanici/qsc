...
void Qsc::init_axis()
{
  ...
  for (n = 1; n < R0c.size(); n++)
  {
    ...
    R0pp += R0c[n] * (-n * nfp * n * nfp) * cosangle + R0s[n] * (-n * nfp * n * nfp) * sinangle;
    Z0pp += Z0c[n] * (-n * nfp * n * nfp) * cosangle + Z0s[n] * (-n * nfp * n * nfp) * sinangle;
    R0ppp += R0c[n] * (n * nfp * n * nfp * n * nfp) * sinangle + R0s[n] * (-n * nfp * n * nfp * n * nfp) * cosangle;
    Z0ppp += Z0c[n] * (n * nfp * n * nfp * n * nfp) * sinangle + Z0s[n] * (-n * nfp * n * nfp * n * nfp) * cosangle;
  }
...
#define d3_r_d_phi3_cylindrical3 Z0ppp
  ...
  d_tangent_d_l_cylindrical1 = (-d_r_d_phi_cylindrical1 * d2_l_d_phi2 / d_l_d_phi + d2_r_d_phi2_cylindrical1) / (d_l_d_phi * d_l_d_phi);

  d_tangent_d_l_cylindrical2 = (-d_r_d_phi_cylindrical2 * d2_l_d_phi2 / d_l_d_phi + d2_r_d_phi2_cylindrical2) / (d_l_d_phi * d_l_d_phi);

  d_tangent_d_l_cylindrical3 = (-d_r_d_phi_cylindrical3 * d2_l_d_phi2 / d_l_d_phi + d2_r_d_phi2_cylindrical3) / (d_l_d_phi * d_l_d_phi);
  /*
  ...
  */
  ...
  torsion_numerator = (0 + d_r_d_phi_cylindrical1 * (d2_r_d_phi2_cylindrical2 * d3_r_d_phi3_cylindrical3 - d2_r_d_phi2_cylindrical3 * d3_r_d_phi3_cylindrical2) + d_r_d_phi_cylindrical2 * (d2_r_d_phi2_cylindrical3 * d3_r_d_phi3_cylindrical1 - d2_r_d_phi2_cylindrical1 * d3_r_d_phi3_cylindrical3) + d_r_d_phi_cylindrical3 * (d2_r_d_phi2_cylindrical1 * d3_r_d_phi3_cylindrical2 - d2_r_d_phi2_cylindrical2 * d3_r_d_phi3_cylindrical1));
