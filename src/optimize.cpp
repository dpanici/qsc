...
void Opt::optimize()
{
  ...
  if (vary_R0c.size() != q.R0c.size())
    throw std::runtime_error("Size of vary_R0c is incorrect");
  if (vary_R0s.size() != q.R0s.size())
    throw std::runtime_error("Size of vary_R0s is incorrect");
  if (vary_Z0c.size() != q.Z0c.size())
    throw std::runtime_error("Size of vary_Z0c is incorrect");
  if (vary_Z0s.size() != q.Z0s.size())
    throw std::runtime_error("Size of vary_Z0s is incorrect");
  ...
  for (j_fourier_refine = 0; j_fourier_refine <= fourier_refine; j_fourier_refine++)
  {
    ...
    {
      if (verbose > 0)
        std::cout << "Skipping j_fourier_refine = " << j_fourier_refine << " since n_iter is too large." << std::endl;
      break;
    }
  ...
  if (verbose > 0)
    std::cout << "Goodbye from optimize" << std::endl;
}
...
int gsl_residual_function(const gsl_vector *x, void *params, gsl_vector *f)
{
  ...
  */
  ...
  if (opt->verbose > 1)
    std::cout << "Goodbye from gsl_residual_function" << std::endl
              << std::flush;
  return GSL_SUCCESS;
}
...
void Opt::init_parameters()
{
  ...
  n_parameters = 0;
  if (make_names)
    state_vector_names.resize(0, "");
  if (vary_eta_bar)
  {
    if (make_names)
      state_vector_names.push_back("eta_bar");
    n_parameters++;
  }
  if (vary_sigma0)
  {
    if (make_names)
      state_vector_names.push_back("sigma0");
    n_parameters++;
  }
  if (vary_B2c)
  {
    if (make_names)
      state_vector_names.push_back("B2c");
    n_parameters++;
  }
  if (vary_B2s)
  {
    if (make_names)
      state_vector_names.push_back("B2s");
    n_parameters++;
  }
  for (j = 0; j < vary_R0c.size(); j++)
  {
    ...
    {
      if (make_names)
        state_vector_names.push_back("R0c[" + std::to_string(j) + "]");
      n_parameters++;
    }
  }
  for (j = 0; j < vary_R0s.size(); j++)
  {
    ...
    {
      if (make_names)
        state_vector_names.push_back("R0s[" + std::to_string(j) + "]");
      n_parameters++;
    }
  }
  for (j = 0; j < vary_Z0c.size(); j++)
  {
    ...
    {
      if (make_names)
        state_vector_names.push_back("Z0c[" + std::to_string(j) + "]");
      n_parameters++;
    }
  }
  for (j = 0; j < vary_Z0s.size(); j++)
  {
    ...
    {
      if (make_names)
        state_vector_names.push_back("Z0s[" + std::to_string(j) + "]");
      n_parameters++;
    }
  }
...
void Opt::init_residuals()
{
  ...
  n_terms = 0;
  if (make_names)
    residual_names.resize(0, "");
  if (weight_B20 > 0)
  {
    n_terms += q.nphi;
    if (make_names)
      for (j = 0; j < q.nphi; j++)
        residual_names.push_back("B20[" + std::to_string(j) + "]");
  }
  if (weight_iota > 0)
  {
    n_terms++;
    if (make_names)
      residual_names.push_back("iota");
  }
  if (weight_elongation > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi; j++)
        residual_names.push_back("elongation[" + std::to_string(j) + "]");
  }
  if (weight_curvature > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi; j++)
        residual_names.push_back("curvature[" + std::to_string(j) + "]");
  }
  if (weight_R0 > 0)
  {
    n_terms += q.nphi;
    if (make_names)
      for (j = 0; j < q.nphi; j++)
        residual_names.push_back("R0[" + std::to_string(j) + "]");
  }
  if (weight_d2_volume_d_psi2 > 0)
  {
    n_terms++;
    if (make_names)
      residual_names.push_back("d2_volume_d_psi2");
  }
  if (weight_DMerc_times_r2 > 0)
  {
    n_terms++;
    if (make_names)
      residual_names.push_back("DMerc_times_r2");
  }
  if (weight_XY2 > 0)
  {
    n_terms += q.nphi * 6;
    if (make_names)
      for (j = 0; j < q.nphi * 6; j++)
        residual_names.push_back("XY2[" + std::to_string(j) + "]");
  }
  if (weight_XY2Prime > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi * 6; j++)
        residual_names.push_back("XY2Prime[" + std::to_string(j) + "]");
  }
  if (weight_XY2PrimePrime > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi * 6; j++)
        residual_names.push_back("XY2PrimePrime[" + std::to_string(j) + "]");
  }
  if (weight_Z2 > 0)
  {
    n_terms += q.nphi * 3;
    if (make_names)
      for (j = 0; j < q.nphi * 3; j++)
        residual_names.push_back("Z2[" + std::to_string(j) + "]");
  }
  if (weight_Z2Prime > 0)
  {
    n_terms += q.nphi * 3;
    if (make_names)
      for (j = 0; j < q.nphi * 3; j++)
        residual_names.push_back("Z2Prime[" + std::to_string(j) + "]");
  }
  if (weight_XY3 > 0)
  {
    n_terms += q.nphi * 3;
    if (make_names)
      for (j = 0; j < q.nphi * 3; j++)
        residual_names.push_back("XY3[" + std::to_string(j) + "]");
  }
  if (weight_XY3Prime > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi * 3; j++)
        residual_names.push_back("XY3Prime[" + std::to_string(j) + "]");
  }
  if (weight_XY3PrimePrime > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi * 3; j++)
        residual_names.push_back("XY3PrimePrime[" + std::to_string(j) + "]");
  }
  if (weight_grad_B > 0)
  {
    n_terms += q.nphi * 9;
    if (make_names)
      for (j = 0; j < q.nphi * 9; j++)
        residual_names.push_back("grad_B[" + std::to_string(j) + "]");
  }
  if (weight_grad_grad_B > 0)
  {
    n_terms += q.nphi * 27;
    if (make_names)
      for (j = 0; j < q.nphi * 27; j++)
        residual_names.push_back("grad_grad_B[" + std::to_string(j) + "]");
  }
  if (weight_r_singularity > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi; j++)
        residual_names.push_back("r_singularity[" + std::to_string(j) + "]");
  }
  if (weight_axis_length > 0)
  {
    n_terms += 1;
    if (make_names)
      residual_names.push_back("axis_length");
  }
  if (weight_standard_deviation_of_R > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi; j++)
        residual_names.push_back("standard_deviation_of_R[" + std::to_string(j) + "]");
  }
  if (weight_B20_mean > 0)
  {
    ...
    if (make_names)
      for (j = 0; j < q.nphi; j++)
        residual_names.push_back("B20_mean[" + std::to_string(j) + "]");
  }
...
void Opt::set_residuals(gsl_vector *gsl_residual)
{
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    B20_term += term * term;
    if (weight_B20 > 0)
      residuals[j++] = weight_B20 * term;
  }

  term = q.iota - target_iota;
  iota_term = term * term;
  if (weight_iota > 0)
    residuals[j++] = weight_iota * term;
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_elongation > 0)
      residuals[j++] = weight_elongation * term;
  }
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_curvature > 0)
      residuals[j++] = weight_curvature * term;
  }
  ...
  if (weight_d2_volume_d_psi2 > 0)
    residuals[j++] = weight_d2_volume_d_psi2 * term;
  d2_volume_d_psi2_term = term * term;

  if (q.DMerc_times_r2 < min_DMerc_times_r2)
  {
    term = min_DMerc_times_r2 - q.DMerc_times_r2;
  }
  else
  {
    term = 0.0;
  ...
  if (weight_DMerc_times_r2 > 0)
    residuals[j++] = weight_DMerc_times_r2 * term;
  DMerc_times_r2_term = term * term;
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_XY2Prime > 0)
      residuals[j++] = weight_XY2Prime * term;
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_XY2PrimePrime > 0)
      residuals[j++] = weight_XY2PrimePrime * term;
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_Z2Prime > 0)
      residuals[j++] = weight_Z2Prime * term;
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_XY3Prime > 0)
      residuals[j++] = weight_XY3Prime * term;
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_XY3PrimePrime > 0)
      residuals[j++] = weight_XY3PrimePrime * term;
  ...
  for (k = 0; k < q.nphi; k++)
  {
    for (int j2 = 0; j2 < 9; j2++)
    {
      ...
      if (weight_grad_B > 0)
        residuals[j++] = weight_grad_B * term;
    }
  }
  ...
  for (k = 0; k < q.nphi; k++)
  {
    for (int j2 = 0; j2 < 27; j2++)
    {
      ...
      if (weight_grad_grad_B > 0)
        residuals[j++] = weight_grad_grad_B * term;
    }
  }
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_r_singularity > 0)
      residuals[j++] = weight_r_singularity * term;
  }

  term = q.axis_length - target_axis_length;
  axis_length_term = term * term;
  if (weight_axis_length > 0)
    residuals[j++] = weight_axis_length * term;
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_standard_deviation_of_R > 0)
      residuals[j++] = weight_standard_deviation_of_R * term;
  }
  ...
  for (k = 0; k < q.nphi; k++)
  {
    ...
    if (weight_B20_mean > 0)
      residuals[j++] = weight_B20_mean * term;
  }

  Vector weights = {weight_B20, weight_iota, weight_elongation, weight_curvature, weight_R0,
  ...
  for (k = 0; k < terms.size(); k++)
  {
    ...
    if (weights[k] > 0)
      objective_function += 0.5 * weights[k] * weights[k] * terms[k];
  }
                  ...
                  const gsl_multifit_nlinear_workspace *w)
{
  ...
  opt->iter_DMerc_times_r2_term[n_iter] = opt->DMerc_times_r2_term;
